...
$parentModule = 'DefenderAPI'
if (-not $ModuleName) { $ModuleName = 'DefenderAPI.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\DefenderAPI'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\DefenderAPI.psd1"
$prereqHash = @{
